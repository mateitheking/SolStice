...
declare_id!("yRxrK31ibWBXorgH1g1VVXmDzZotzVhv3AudE48W44f");
...
pub mod solstice {
    ...
    pub fn initialize(ctx: Context<Initialize>) -> Result<()> {
        ctx.accounts.config.agent_authority = pubkey!("2KQpJLLdexkrcSyKGES7fZV3Q3Wvd9umvYgqc6fQ41te");
        ctx.accounts.config.admin = ctx.accounts.admin.key();
        msg!("Config initialized. Agent authority: {}", ctx.accounts.config.agent_authority);
        Ok(())
    }
